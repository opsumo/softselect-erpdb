...
class MySqlDatabase
{
    ...
    const mysqli_DATE_FORMAT = 'Y-m-d';
    const mysqli_TIME_FORMAT = 'H:i:s';
    const mysqli_DATETIME_FORMAT = 'Y-m-d H:i:s';
...
//        if ($database) $this->useDatabase($database);
     ...
     *  Iterate Result Set
     *
     *  Returns a {@link mysqli_ResultSet} iteratable object for a query. The $type
     ...
     *  foreach ($db->iterate($sql_query, mysqli_Db::DATA_ARRAY) as $row) {
     *      echo $row[0] . '<br/>';
     *      echo $row['col1'] . '<br/>';
     *  }
     *  </code>
    ...
    public function quickQuery($query)
    {
        ...
        if (!$r) return false;
        if (is_resource($r)) mysqli_free_result($r);
